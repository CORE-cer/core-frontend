...
export async function getQueryInfos(): Promise<QueryIdToQueryInfoMap> {
  ...
  if (!queriesParse.success) {
    throw new Error(
      "Failed to parse query info: " + queriesParse.error.message,
    );
  }

  const queries = queriesParse.data;

  const activeQueryInfos = queries.filter(
    (query) => query.active && query.query_id !== null,
  );
...
export async function getStreamsInfo(): Promise<StreamInfo[]> {
  ...
  const streamsInfoParse =
    GetStreamInfoSchema.array().safeParse(streamsInfoUntyped);

  if (!streamsInfoParse.success) {
    throw new Error(
      "Failed to parse streams info: " + streamsInfoParse.error.message,
    );
  }
...
export async function inactivateQuery({
  queryId,
}: {
  queryId: QueryId;
}): Promise<void> {
...
export const addQuery = async ({
  ...
  if (!createQueryParse.success) {
    throw new Error(
      "Failed to create new query due to error: " +
        createQueryParse.error.message,
    );
  }
