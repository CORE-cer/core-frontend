...
function WatchPageContent() {
  const { viewMode, handleViewModeChange } = useWatchPageContext();
  const [chartLayoutMode, setChartLayoutMode] = useState<"list" | "freeform">(
    "list",
  );
      ...
      <Box sx={{ display: "flex", width: "100%", height: "100%" }}>
        ...
        >
          ...
          >
            ...
            {viewMode === "charts" && (
              <Tooltip
                title={`Switch to ${chartLayoutMode === "list" ? "freeform" : "list"} layout`}
              >
                <IconButton
                  onClick={() => {
                    setChartLayoutMode((prev) =>
                      prev === "list" ? "freeform" : "list",
                    );
                  }}
                  size="small"
                >
