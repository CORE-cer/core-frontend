...
export function WatchPageProvider({ children }: { children: React.ReactNode }) {
  ...
  return <WatchPageContext.Provider value={watchPage}>{children}</WatchPageContext.Provider>;
}

export function useWatchPageContext(): WatchPageContextType {
  const context = useContext(WatchPageContext);
  if (!context) {
    throw new Error('useWatchPageContext must be used within a WatchPageProvider');
  }
  return context;
}
